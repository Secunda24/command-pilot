"""
jarvis_ui.py  v4
Tkinter UI — all canvas updates routed through after() for thread safety.
Starts the OS action queue drain on the main thread.
"""
...
class JarvisUI:
    def __init__(self, master):
        ...
        self.colors = {
            'primary':'#00D4FF','secondary':'#0080FF','glow':'#66E5FF',
            'dark':'#001122','bg':'#000510','red':'#FF3333','green':'#33FF33'
        }
        ...
        self.jarvis = JarvisCore(ui_callback=self._set_status, master=self.master)
    ...
    def _build_canvas(self):
        ...
        self.canvas.create_text(
            cx,580,text="[ESC] Exit  |  Say 'hello' to wake",
            fill=self.colors['secondary'],font=("Consolas",10))
    ...
    def _apply_status(self, status, text=""):
        self.status = status
        c = {'STANDBY':self.colors['primary'], 'LISTENING':self.colors['red'],
             'SPEAKING':self.colors['green'],  'THINKING':self.colors['glow']
    ...
    # ── animation ─────────────────────────────────────────────────────────────
    def _animate(self):
        self.angle += 1.5;  self.pulse += 0.08
        self.canvas.itemconfig(self.time_text, text=datetime.now().strftime("%H:%M:%S"))
